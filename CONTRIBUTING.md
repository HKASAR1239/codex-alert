...
For macOS-specific changes, also check `bash install.command --check` and test the relevant behavior on a Mac. Unit tests must use synthetic sessions and mocked notification requests, never real conversations or real ntfy topics. Update both READMEs if setup or behavior changes.
