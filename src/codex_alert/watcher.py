...
class Watcher:
    ...
    contains file cursors, active task start times, and terminal task IDs since
    ...
    def poll(self) -> list[dict]:
        ...
        records: list[dict] = []
        recent_cutoff = time.time() - RECENT_FILE_SECONDS
        for path in sorted(self.session_root.rglob("*.jsonl")):
            ...
            if previous is None and stat.st_mtime < recent_cutoff:
                ...
                continue
            try:
                updated, new_records = self._read_file(path, previous)
            except OSError:
    ...
    def _read_file(self, path: Path, previous: dict | None) -> tuple[dict, list[dict]]:
        records = []
